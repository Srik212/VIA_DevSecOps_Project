...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:admin123@localhost:5432/inventorydb'
...
def search():
    ...
    # VULNERABLE: XSS (unsanitized user input rendered)
...
def upload():
    if request.method == 'POST':
        ...
        # VULNERABLE: Extension unrestricted & size unrestricted
        filepath = os.path.join('uploads', file.filename)
        file.save(filepath)
        
        flash("File uploaded successfully.")
        return redirect('/dashboard')  # Open Redirect possible with user-controlled input
...
@app.route('/include')
def include():
    # VULNERABLE: Dangerous file inclusion (use only with EXTREME caution!)
    filename = request.args.get('page', 'home')
    with open(f'templates/{filename}.html') as f:
        content = f.read()
    return content
...
# Inventory Management System with Vulnerabilities
from flask import Flask, render_template, redirect, url_for, request, flash, session
from flask_sqlalchemy import SQLAlchemy
from flask_bcrypt import Bcrypt
from flask_login import LoginManager, UserMixin, login_user, login_required, logout_user, current_user
...
app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv(
    'DATABASE_URL',
    'postgresql://postgres:admin123@db:5432/inventorydb'
)
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
